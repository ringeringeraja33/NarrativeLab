...
import { ProjectStructureTools } from '../components/ProjectStructureTools';
...
import { Scene, TIMELINE_MODES, TIMELINE_MODE_ICONS, TIMELINE_MODE_LABELS, TimelineMode, formatSceneLength, getStatusOrder, resolveStatusCfg } from '../models/Scene';
...
export class TimelineView extends ProjectBoundItemView {
    ...
    constructor(leaf: WorkspaceLeaf, plugin: SceneCardsPlugin, sceneManager: SceneManager,
        private readonly pageType: string = TIMELINE_VIEW_TYPE,
        private readonly pageMode: 'timeline' | 'tracks' | 'templates' = 'timeline') {
        super(leaf);
        this.plugin = plugin;
        ...
        this.swimlaneMode = pageMode === 'tracks';
    ...
    private renderView(container: HTMLElement): void {
        ...
        if (this.pageMode === 'templates') {
            const content = container.createDiv('nl-chapter-templates-page');
            this.renderStructureEditor(content);
            return;
        }
        ...
        const orderSelect = controls.createEl('select', {
            ...
            attr: { 'aria-label': t('Scene ordering') },
        });
        ...
        if (this.swimlaneMode) {
            const groupSelect = controls.createEl('select', {
                ...
                attr: { 'aria-label': t('Group lanes by') },
            });
    ...
    private renderTimeline(container: HTMLElement): void {
        ...
        if (scenes.length === 0 && definedActs.length === 0) {
            ...
            empty.createEl('p', { text: t('Click "+ New Scene" to create your first scene, or use the structure button to set up acts and chapters.') });
            return;
        }
        ...
        const renderTLItem = (item: TLItem) => {
            if (item.kind === 'divider') {
                ...
                card.createDiv({ cls: 'timeline-card-title', text: t('No scenes in {act}', { act: item.actLabel }) });
                card.createEl('p', { cls: 'timeline-card-hint', text: t('Click + to add a scene') });
    ...
    private renderSwimlaneTimeline(container: HTMLElement): void {
        ...
        if (scenes.length === 0) {
            const empty = timelineEl.createDiv('story-line-empty');
            empty.createEl('p', { text: t('No scenes found.') });
            empty.createEl('p', { text: t('Click "+ New Scene" to create your first scene.') });
            return;
        }
    ...
    private renderSwimlaneCard(cell: HTMLElement, scene: Scene): void {
        ...
        const statusBadge = footer.createSpan({ cls: 'timeline-card-status', text: t(statusCfg.label) });
    ...
    private showContextMenu(scene: Scene, event: MouseEvent): void {
        ...
        menu.addItem(item => {
            item.setTitle(t('Edit Date/Time'))
                .setIcon('clock')
    ...
    openStructureModal(): void {
        new ProjectStructureTools(this.app, this.plugin, this.sceneManager).open();
    }

private renderStructureEditor(contentEl: HTMLElement, onDone?: () => void): void {
        new ProjectStructureTools(this.app, this.plugin, this.sceneManager).render(contentEl, onDone);
    }
    ...
    private openTimeEditModal(scene: Scene): void {
        ...
        modal.titleEl.setText(t('Time & Order — {title}', { title: scene.title || t('Untitled') }));
        ...
        new Setting(modal.contentEl)
            .setName(t('Date / Day'))
            .setDesc(t('E.g. 2026-02-17, Day 1, Monday, Chapter 3…'))
            .addText((text: TextComponent) => {
                text.setValue(storyDate)
                    .setPlaceholder(t('e.g. Day 1'))
        ...
        new Setting(modal.contentEl)
            .setName(t('Time'))
            .setDesc(t('E.g. 14:00, morning, evening, night…'))
            .addText((text: TextComponent) => {
                text.setValue(storyTime)
                    .setPlaceholder(t('e.g. evening'))
                    .onChange((v: string) => (storyTime = v));
            });

        // Legacy timeline field
        new Setting(modal.contentEl)
            .setName(t('Timeline note'))
            .setDesc(t('Free-form note about when this happens in the story'))
            .addText((text: TextComponent) => {
                text.setValue(timeline)
                    .setPlaceholder(t('e.g. After the party'))
                    .onChange((v: string) => (timeline = v));
            });

        // Chronological order field
        new Setting(modal.contentEl)
            .setName(t('Chronological order'))
        ...
        new Setting(modeSection)
            .setName(t('Timeline mode'))
            .setDesc(t('How the plugin handles this scene\'s temporal position'))
        ...
        new Setting(modal.contentEl)
            .addButton((btn: ButtonComponent) => {
                btn.setButtonText(t('Save')).setCta().onClick(async () => {
