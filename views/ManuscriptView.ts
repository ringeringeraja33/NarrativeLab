...
export class ManuscriptView extends ProjectBoundItemView {
    ...
    private renderView(container: HTMLElement): void {
        ...
        if (filterBar) {
            ...
            const plainWrap = (filterBar as HTMLElement).createEl('label', {
                cls: 'sl-toggle-wrap',
                attr: { title: t('Hide link and tag styling so the manuscript reads as plain prose') },
            });
            plainWrap.createSpan({ cls: 'sl-toggle-label', text: t('Hide link styles') });
            ...
            const lockWrap = (filterBar as HTMLElement).createEl('label', {
                cls: 'sl-toggle-wrap',
                attr: { title: t('Treat wiki-links and tags as whole units so the cursor cannot edit inside them') },
            });
            lockWrap.createSpan({ cls: 'sl-toggle-label', text: t('Protect links') });
    ...
    private async renderManuscript(): Promise<void> {
        ...
        if (scenes.length === 0) {
            this.scrollArea.createDiv({
                ...
                text: t('No scenes match the current filters.'),
            });
        ...
        this.footerEl.setText(t('{n} scenes · {words} {unit}', { n: scenes.length, words: totalWords.toLocaleString(), unit: wordLabel }));
    ...
    private async mountReadOnlyPreview(
        ...
        if (text) {
            ...
            container.createDiv({ cls: 'sl-manuscript-scene-empty', text: t('Empty scene') });
        }
    }
    ...
    private buildSearchPanel(container: HTMLElement): void {
        ...
        const input = row.createEl('input', {
            ...
            attr: { type: 'text', placeholder: t('Find in manuscript…'), 'aria-label': t('Find in manuscript') },
        });
        ...
        const nextBtn = row.createEl('button', { cls: 'sl-manuscript-search-btn', attr: { 'aria-label': t('Next match'), title: t('Next (Enter)') } });
        setIcon(nextBtn, 'chevron-down');
        nextBtn.addEventListener('click', () => this.searchNext(true));

        const caseBtn = row.createEl('button', { cls: 'sl-manuscript-search-toggle', attr: { 'aria-label': t('Match case'), title: t('Match case') } });
        caseBtn.setText('Aa');
        ...
        const wordBtn = row.createEl('button', { cls: 'sl-manuscript-search-toggle', attr: { 'aria-label': t('Whole word'), title: t('Whole word') } });
        ...
        const regexBtn = row.createEl('button', { cls: 'sl-manuscript-search-toggle', attr: { 'aria-label': t('Regular expression'), title: t('Regex') } });
        regexBtn.setText('.*');
        ...
        const closeBtn = row.createEl('button', { cls: 'sl-manuscript-search-btn', attr: { 'aria-label': t('Close'), title: t('Close (Esc)') } });
        setIcon(closeBtn, 'x');
        ...
        const replaceInput = replaceRow.createEl('input', {
            cls: 'sl-manuscript-search-input',
            attr: { type: 'text', placeholder: t('Replace…'), 'aria-label': t('Replace') },
        });
        this.replaceInput = replaceInput;

        const replaceBtn = replaceRow.createEl('button', { cls: 'sl-manuscript-search-btn', attr: { 'aria-label': t('Replace this match'), title: t('Replace') } });
        setIcon(replaceBtn, 'replace');
        replaceBtn.addEventListener('click', () => this.replaceCurrent());

        const replaceAllBtn = replaceRow.createEl('button', { cls: 'sl-manuscript-search-btn', attr: { 'aria-label': t('Replace all matches'), title: t('Replace all') } });
    ...
    private async replaceAll(): Promise<void> {
        ...
        new Notice(t('Replaced {n} occurrence(s)', { n: totalReplaced }));
    ...
    //
    // When the user switches to another NarrativeLab view (Codex, Plotgrid, etc.)
