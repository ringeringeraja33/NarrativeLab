 ...
 * if both NarrativeLab and ToWord are installed simultaneously.
 */
...
// ── Settings interface (NarrativeLab-scoped) ──────────────────────
    ...
    private async processChunkedConversion(
        ...
        for (let i = 0; i < chunks.length; i++) {
            try {
                ...
                console.error(`NarrativeLab DOCX: Error processing chunk ${i + 1}:`, error);
            }
        }
    ...
    private generateDocx(elements: SLDocumentElement[], _title: string): Promise<Blob> {
        ...
        return new Promise((resolve, reject) => {
            zip(files, { level: 6 }, (err, data) => {
                if (err) {
                    console.error('NarrativeLab DOCX: ZIP creation failed:', err);
                    reject(err);
                } else {
  ...
  <w:body>
    ...
    private elementToXml(element: SLDocumentElement): string {
        try {
            ...
            console.error('NarrativeLab DOCX: Error converting element to XML:', element.type, error);
  ...
  <w:r>
            ...
            if (standardImageMatch) {
                ...
                if (url.startsWith('http://') || url.startsWith('https://')) {
                    try {
                        ...
                        console.error('NarrativeLab DOCX: Error fetching external image:', error);
                    }
                } else if (this.resourceLoader) {
                    try {
                        imageData = await this.resourceLoader(url);
                    } catch (error) {
                        console.error(`NarrativeLab DOCX: Error loading local image ${url}:`, error);
                    }
                }
            ...
            if (wikiImageMatch) {
                ...
                if (this.resourceLoader) {
                    try {
                        ...
                        console.error(`NarrativeLab DOCX: Error loading file ${fileName}:`, error);
                    }
                }
    ...
    private getImageDimensions(imageData: ArrayBuffer): { width: number; height: number } {
        try {
            ...
            console.warn('NarrativeLab DOCX: Error reading image dimensions:', error);
        }
