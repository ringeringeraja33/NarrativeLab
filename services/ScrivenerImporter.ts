 ...
 *  4. Create a new NarrativeLab project with appropriate folders
...
interface BinderItem {
    ...
    /** Scrivener custom metadata → NarrativeLab custom fields */
...
// ────────────────────────────────────────────────────
//  Scrivener status → NarrativeLab status mapping
// ────────────────────────────────────────────────────
...
/** Map known Scrivener icon filenames to NarrativeLab import categories. */
...
export class ScrivenerImporter {
    ...
    async import(scrivPath: string): Promise<ImportResult> {
        ...
        if (isSeries) {
            ...
            const seriesFolder = normalizePath([root, safeName].filter(Boolean).join('/'));
