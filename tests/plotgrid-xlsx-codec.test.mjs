...
test('embedded Univer host exposes the legacy grid view controls', async () => {
    ...
    assert.match(host, /LINK_HOVER_MS\s*=\s*3000/);
    assert.match(host, /onShowConnectedNotesHover/);
    assert.match(view, /showLinkHoverCard/);
    assert.match(view, /plot-grid-link-hover/);
