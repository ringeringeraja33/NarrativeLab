...
function renderOneSection<T extends { custom?: Record<string, string> }>(
...

    {
        ...
        attachTooltip(moveUpBtn, t('Move section up'));
        ...
        attachTooltip(moveDownBtn, t('Move section down'));
        ...
        attachTooltip(renameBtn, t('Rename section'));
        ...
        attachTooltip(deleteBtn, t('Remove section'));
        ...
        for (let fIdx = 0; fIdx < sec.fields.length; fIdx++) {
                    ...
                    inp.addEventListener('keydown', (e) => {
                        ...
                        if (e.key === 'Enter') {
                            ...
                            if (allowed.length > 0 && !allowed.includes(v)) {
                                new Notice(t('"{value}" is not one of the allowed options.', { value: v }));
                                return;
                            }
            ...
            editBtn.addEventListener('click', () => {
                const modal = new AddSectionFieldModal(app, (result) => {
                    ...
                    if (trimmedName !== fname && sec.fields.some(f => fieldName(f) === trimmedName)) {
                        new Notice(t('Field "{name}" already exists in this section.', { name: trimmedName }));
                        return;
                    }
            ...
            attachTooltip(moveSectionBtn, t('Move field to section'));
                    ...
                    menu.addItem(item => item
                        ...
                        .onClick(() => {
                            if (target.fields.some(f => fieldName(f) === fname)) {
                                new Notice(t('Field "{name}" already exists in section "{section}".', { name: fname, section: target.title }));
                                return;
                            }
...
export class AddCustomSectionModal extends Modal {
    ...
    onOpen(): void {
        ...
        new Setting(this.contentEl)
            .setName(t('Section title'))
            .addText(text => {
                text.setPlaceholder(t('e.g. Misbelief, Voice, Wardrobe…'));
...
export class AddSectionFieldModal extends Modal {
    ...
    onOpen(): void {
        ...
        new Setting(this.contentEl)
            ...
            .addText(text => {
                text.setPlaceholder(t('e.g. The Lie, The Truth…'));
                text.setValue(name);
        ...
        const refreshOptionsRow = () => {
            ...
            new Setting(optionsContainer)
                .setName(t('Options'))
                .setDesc(t('Comma-separated list of choices.'))
                .addTextArea(ta => {
                    ta.setPlaceholder(t('option 1, option 2, option 3'));
            ...
            new Setting(optionsContainer)
                ...
                .setDesc(t('Vault folder path whose note names become selectable options.'))
                .addText(text => {
        ...
        this.submit = () => {
            ...
            if (!result) {
                new Notice(t('Please enter a field name.'));
                return;
            }
            this.close();
