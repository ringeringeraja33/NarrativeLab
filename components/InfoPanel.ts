...
export class InfoPanelComponent {
    ...
    /** Serialize notes autosaves so overlapping read→modify cannot clobber newer text. */
    ...
    private renderStatusRow(scene: Scene): void {
        ...
        row.createSpan({ cls: 'sl-info-label', text: t('Status') });
        ...
        button.createSpan({ cls: 'sl-info-status-label', text: t(currentCfg.label) });
    ...
    private renderPovRow(scene: Scene): void {
        const row = this.container.createDiv('sl-info-row');
        row.createSpan({ cls: 'sl-info-label', text: t('POV') });
    ...
    private renderLocationRow(scene: Scene): void {
        const row = this.container.createDiv('sl-info-row');
        row.createSpan({ cls: 'sl-info-label', text: t('Location') });
        const field = row.createDiv('sl-info-field');
        renderAutocompleteInput({
            container: field,
    ...
    private renderWordCountRow(scene: Scene): void {
        const row = this.container.createDiv('sl-info-row');
        row.createSpan({ cls: 'sl-info-label', text: t('Words') });
    ...
    private renderNotes(scene: Scene): void {
        const section = this.container.createDiv('sl-info-section');
        section.createDiv({ cls: 'sl-info-section-label', text: t('Notes') });
    ...
    private renderNotesLive(container: HTMLElement, scene: Scene): void {
        ...
        if (!scene.notes) {
            ...
            placeholder.createDiv({ cls: 'sl-info-notes-placeholder', text: t('Click to add notes…') });
            placeholder.addEventListener('click', () => {
    ...
    private renderNotesEditor(container: HTMLElement, scene: Scene): void {
        ...
        const textarea = editorEl.createEl('textarea', {
            ...
            attr: { placeholder: t('Write notes in markdown… Use - [ ] for checkboxes, **bold**, [[wikilinks]]') },
        });
