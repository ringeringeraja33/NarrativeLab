...
export class InspectorComponent {
    ...
    private render(): void {
        ...
        header.createEl('h3', { text: t('Scene Details') });
        ...
        const titleInput = titleSection.createEl('input', {
            ...
            attr: { type: 'text', placeholder: t('Scene title…') },
        });
        ...
        titleInput.addEventListener('change', async () => {
            ...
            if (val && val !== scene.title) {
                ...
                if (updated > 0) {
                    new Notice(t('Updated {n} setup/payoff link(s)', { n: updated }));
                }
            }
        });
        ...
        const subtitleInput = titleSection.createEl('input', {
            ...
            attr: { type: 'text', placeholder: t('Subtitle (optional)…') },
        });
        ...
        actGroup.createSpan({ cls: 'inspector-label', text: t('Act') });
        ...
        actInput.addEventListener('change', async () => {
            ...
            if (typeof val === 'string' && actChapterHasIllegalPathChars(val)) {
                new Notice(t("Act name contains characters that aren't allowed in folder names; they'll be replaced with '-'."));
            }
        ...
        chGroup.createSpan({ cls: 'inspector-label', text: t('Chapter') });
        ...
        chInput.addEventListener('change', async () => {
            ...
            if (typeof val === 'string' && actChapterHasIllegalPathChars(val)) {
                new Notice(t("Chapter name contains characters that aren't allowed in folder names; they'll be replaced with '-'."));
            }
        ...
        seqGroup.createSpan({ cls: 'inspector-label', text: t('Sequence') });
        ...
        chronoSection.createSpan({ cls: 'inspector-label', text: t('Chronological Order: ') });
        const chronoInput = chronoSection.createEl('input', { attr: { type: 'number', placeholder: t('Same as sequence if blank') } });
        ...
        povSection.createSpan({ cls: 'inspector-label', text: t('POV: ') });
        ...
        charSection.createSpan({ cls: 'inspector-label', text: t('Characters:') });
        ...
        tmGroup.createSpan({ cls: 'inspector-label', text: t('Timeline Mode') });
        ...
        strandGroup.createSpan({ cls: 'inspector-label', text: t('Strand') });
        const strandInput = strandGroup.createEl('input', { attr: { type: 'text', placeholder: t('e.g. "1943", "outer"') } });
        ...
        dateGroup.createSpan({ cls: 'inspector-label', text: t('Story Date') });
        const dateInput = dateGroup.createEl('input', { attr: { type: 'text', placeholder: t('e.g. 2026-02-17, Day 3') } });
        ...
        timeGroup.createSpan({ cls: 'inspector-label', text: t('Story Time') });
        const timeInput = timeGroup.createEl('input', { attr: { type: 'text', placeholder: t('e.g. morning, 14:00') } });
        ...
        targetGroup.createSpan({ cls: 'inspector-label', text: t('Target') });
        ...
        tagSection.createSpan({ cls: 'inspector-label', text: t('Plotlines / Tags:') });
        ...
        // v1.10.17.)
        {
            ...
            synSection.createSpan({ cls: 'inspector-label', text: t('Synopsis:') });
            const synInput = synSection.createEl('textarea', {
                cls: 'inspector-synopsis-input',
                attr: { placeholder: t('One- or two-sentence summary of the scene…'), rows: '6' },
            });
        ...
        // ── Scene Draft (body text — editable) ──
        {
            ...
            descSection.createSpan({ cls: 'inspector-label', text: t('Scene Draft:') });
            const descInput = descSection.createEl('textarea', {
                cls: 'inspector-description-input',
                attr: { placeholder: t('Write your scene draft here…'), rows: '12' },
            });
        ...
        conflictSection.createSpan({ cls: 'inspector-label', text: t('Conflict:') });
        const conflictInput = conflictSection.createEl('textarea', {
            cls: 'inspector-conflict-input',
            attr: { placeholder: t('What is the main conflict?'), rows: '12' },
        });
        ...
        emotionSection.createSpan({ cls: 'inspector-label', text: t('Emotion: ') });
        const emotionInput = emotionSection.createEl('input', {
            cls: 'inspector-emotion-input',
            attr: { type: 'text', placeholder: t('e.g. tense, hopeful, melancholic') },
        });
        ...
        intensitySection.createSpan({ cls: 'inspector-label', text: t('Intensity: ') });
    ...
    private renderUniversalFields(scene: Scene): void {
        ...
        header.createSpan({ cls: 'inspector-label', text: t('Custom Fields') });

        const addBtn = header.createEl('button', {
            cls: 'inspector-universal-add-btn clickable-icon',
            attr: { title: t('Add custom field'), 'aria-label': t('Add custom field') },
        });
    ...
    private renderSingleUniversalField(
        ...
        const editBtn = labelWrap.createEl('button', {
            ...
            attr: { title: t('Edit or remove this field'), 'aria-label': t('Edit field') },
        });
    ...
    private renderSetupPayoff(scene: Scene): void {
        ...
        section.createSpan({ cls: 'inspector-label', text: t('Setup / Payoff:') });
        ...
        payoffLabel.createSpan({ text: t(' Sets up:'), cls: 'inspector-sp-label' });
        ...
        setupLabel.createSpan({ text: t(' Set up by:'), cls: 'inspector-sp-label' });
        ...
        if (scene.payoff_scenes?.length) {
            ...
            if (dangling.length > 0) {
                ...
                warn.createSpan({ text: t('Missing payoff target: {targets}', { targets: dangling.join(', ') }) });
            }
        }
    }
    ...
    private renderDetectedLinks(scene: Scene): void {
        ...
        headerRow.createSpan({ cls: 'inspector-label', text: t(' Detected in text') });
    ...
    private showTagTypeMenu(e: MouseEvent, tagName: string, onUpdate: () => void): void {
        ...
        if (sceneForIgnore) {
            menu.addItem(item => {
                item.setTitle(t('Ignore in this scene'))
                    .setIcon('eye-off')
                    .onClick(async () => {
    ...
    private renderNotes(scene: Scene): void {
        ...
        labelRow.createSpan({ cls: 'inspector-label', text: t(' Notes / Comments') });

        // "Open notes file" button — creates/opens an external .md file
        const openFileBtn = labelRow.createEl('button', {
            cls: 'clickable-icon',
            attr: { title: t('Open as separate notes file'), 'aria-label': t('Open notes file') },
        });
    ...
    private async renderInspectorNotesLive(container: HTMLElement, scene: Scene): Promise<void> {
        ...
        if (!notesText) {
            ...
            placeholder.createDiv({ cls: 'inspector-notes-placeholder', text: t('Click to add notes…') });
    ...
    private renderInspectorNotesEditor(container: HTMLElement, scene: Scene, currentNotes: string): void {
        ...
        const textarea = editorEl.createEl('textarea', {
            ...
            attr: { placeholder: t('Write notes in markdown… Use - [ ] for checkboxes, **bold**, [[wikilinks]]'), rows: '4' },
        });
    ...
    private renderSnapshots(scene: Scene): void {
        ...
        const loadList = async () => {
            ...
            if (snapshots.length === 0) {
                listEl.createSpan({ cls: 'inspector-sp-empty', text: t('No snapshots yet') });
                return;
            }
            for (const snap of snapshots) {
                ...
                const viewBtn = btns.createEl('button', {
                    cls: 'clickable-icon',
                    attr: { title: t('Open snapshot in a new tab (compare)') },
                });
                ...
                const restoreBtn = btns.createEl('button', {
                    cls: 'clickable-icon',
                    attr: { title: t('Restore this snapshot') },
                });
...
class SnapshotLabelModal extends Modal {
    ...
    onOpen(): void {
        ...
        contentEl.createEl('h3', { text: t('Save Snapshot') });
        contentEl.createEl('p', { text: t('Enter a name for this snapshot (e.g. "before major rewrite")') });

        const input = contentEl.createEl('input', {
            attr: { type: 'text', placeholder: t('Snapshot label…') },
