...
export class SceneCardComponent {
    ...
    render(scene: Scene, container: HTMLElement, options?: {
        ...
        if (scene.inactive) {
            ...
            card.setAttribute('aria-label', t('{title}, inactive scene', {
        ...
        if (scene.inactive) {
            titleEl.createSpan({ cls: 'scene-card-inactive-badge', text: t('Inactive') });
        }
        ...
        if (!options?.compact && scene.arcAnchor) {
            ...
            arcBadge.createSpan({ text: t(' Arc Point') });
        }
    ...
    static openColorPicker(app: App, scene: Scene, sceneManager: SceneManager, onDone: () => void): void {
        ...
        modal.titleEl.setText(t('Scene Color'));
        ...
        if (scene.color) {
            const clearBtn = btnRow.createEl('button', { text: t('Clear Color') });
