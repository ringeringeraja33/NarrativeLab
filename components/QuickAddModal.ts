...
export class QuickAddModal extends Modal {
    ...
    onOpen() {
        ...
        new Setting(contentEl)
            .setName(t('Title'))
            .addText(text => {
                text.setPlaceholder(t('Scene title...'))
        ...
        actGroup.createEl('label', { text: t('Act'), cls: 'story-line-field-label' });
        const actSelect = actGroup.createEl('select', { cls: 'dropdown story-line-field-input' });
        actSelect.createEl('option', { text: t('None'), value: '' });
        ...
        actSelect.createEl('option', { text: t('Prologue'), value: '0' });
        for (let i = 1; i <= actCount; i++) {
            actSelect.createEl('option', { text: t(getActDisplayLabel(i)), value: String(i) });
        }
        // Add Epilogue option at the bottom
        actSelect.createEl('option', { text: t('Epilogue'), value: '99' });
        if (this.result.act != null) {
            ...
            if (!isPureNumericActChapter(this.result.act)) {
                actSelect.createEl('option', { text: t(getActDisplayLabel(this.result.act)), value: String(this.result.act) });
            }
        ...
        chapterGroup.createEl('label', { text: t('Chapter'), cls: 'story-line-field-label' });
        ...
        chapterSelect.createEl('option', { text: t('None'), value: '' });
        ...
        chapterSelect.createEl('option', { text: t('+ New chapter ({n})', { n: nextChapter }), value: String(nextChapter) });
        ...
        const povSetting = new Setting(contentEl).setName(t('POV Character'));
        ...
        const charSetting = new Setting(contentEl).setName(t('Characters'));
        ...
        new Setting(contentEl)
            .setName(t('Scene Draft'))
            .addTextArea(area => {
                area.setPlaceholder(t('Write your scene draft here…'))
        ...
        conflictToggle.createSpan({ text: t('Same as description') });

        const conflictSetting = new Setting(conflictWrapper)
            .setName(t('Conflict'))
            .addTextArea(area => {
                area.setPlaceholder(t('What is the main conflict?'))
        ...
        const tagSetting = new Setting(contentEl).setName(t('Tags / Plotlines'));
        ...
        new Setting(contentEl)
            ...
            .addDropdown(dropdown => {
                ...
                statuses.forEach(s => dropdown.addOption(s, t(cfg[s]?.label ?? (s.charAt(0).toUpperCase() + s.slice(1)))));
        ...
        createEditBtn.addEventListener('click', () => {
            if (!this.result.title) {
                new Notice(t('Please enter a scene title'));
                return;
            }
            this.prepareResult();
