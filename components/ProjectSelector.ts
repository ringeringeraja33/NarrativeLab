...
export function renderProjectSelector(
    ...
    if (active?.seriesId && plugin.sceneManager.isProjectInValidSeries(active)) {
        const seriesBadge = wrapper.createSpan({
            cls: 'project-selector-series-badge',
            text: active.seriesId,
        });
        obsidian.setIcon(seriesBadge.createSpan({ cls: 'project-selector-series-icon', prepend: true }), 'library-big');
    }

    // Cover image thumbnail — clickable to pick/change
    if (active) {
        const coverEl = wrapper.createDiv({ cls: 'project-selector-cover' });
        attachTooltip(coverEl, t('Click to set cover image'));
        ...
        coverEl.addEventListener('click', async () => {
            const attachmentSourcePath = active.filePath || plugin.sceneManager.getAttachmentSourcePath();
            const result = await pickImage(plugin.app, attachmentSourcePath, active.coverImage);
    ...
    const newBtn = wrapper.createEl('button', {
        ...
        attr: { 'aria-label': t('New NarrativeLab project'), title: t('New project') },
    });
