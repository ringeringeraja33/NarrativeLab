...
const ZH_CORE: Record<string, string> = {
    ...
    'When the NarrativeLab toolbar is too narrow to fit every view-tab label, show only the icon (Corkboard, Timeline, etc.). Disable to always show both icon and text — the labels will wrap or be clipped if the toolbar is small.': '当 NarrativeLab 工具栏过窄，无法容纳所有视图标签文字时，仅显示图标（平铺看板、时间线等）。关闭后将始终同时显示图标和文字；工具栏较小时，标签可能换行或被截断。',
    ...
    'Show notes in Kanban': '在列式看板中显示笔记',
    'When enabled, corkboard notes are also visible in Kanban columns': '启用后，平铺看板笔记也会显示在列式看板的各列中',
    'Show scenes in Corkboard': '在平铺看板中显示场景',
    'When enabled, scene cards are visible on the corkboard alongside notes': '启用后，场景卡片会与笔记一起显示在平铺看板中',
